...
def get_cost_of_delay_for_sprint(project_id, sprint_id, auth_token):
    ...
    # get issues of a project. -> https://api.taiga.io/api/v1/issues?project=1525740  (completed)
    
    # group issues based on milestone (completed)
    # if milestone == sprint_id then issues.append(issue_id) (completed)
     
    #res = {} (completed)
    #res["0"] = 0 (completed)
     
    # loop through dates of sprint (completed)
        # initialize res[date] = 0 (completed)

    # loop through issues (completed)
        # stories = https://api.taiga.io/api/v1/issues/custom-attributes-values/{issue_id} (completed)
        # storyList = stories.split(','). (completed)
        # dependencies[issue_id] = storyList
    
    # loop through dependencies
        # loop through storyList for each issue_id
            # take the userstory #id and match the #id with ref in userstory to get the actual userstory_id
            # get storypoints/business_value (val) with userstory_id
            # if story_created_date < issue_created_date 
            # res[issue_created_date] += val
    
    # calculate cumulative 
    # prev_date = 0
    # loop through res dates
        # res[date] += res[prev_date]
        # prev_date = date
        ...
        for userstory_label in dependency_value :
            ...
            if userstory_label in userstory_ref_map:
                ...
                if userstory['created_date'] < issue_data[dependency_key]['created_date'] :
                    ...
                    # print(custom_attribute_type_id)

                    # print(custom_attribute_data)
