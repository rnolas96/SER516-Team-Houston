...
export default function Burndown() {
  ...
  const [partialStoryPointAllSprintData, setPartialStoryPointAllSprintData] =
    useState(null);
  const [multiChartAllSprintData, setMultiChartAllSprintData] = useState(null);
          ...
          datasets: [
            {
              ...
              data: values,
              borderColor:
                scenario == "business value for all sprints"
                  ? "blue"
                  : scenario == "Partial Storypoints for all sprints"
                  ? "green"
                  : "black",
  ...
  function setProjectDetails() {
    ...
    axios
      ...
      .then((result) => {
        ...
        setShowLoader(true);
        if (tab == 0) setIsSprintDisabled(false);
      });
  }
  ...
  useEffect(() => {
    const callApis = () => {
      ...
      if (authToken && projectId && sprintId) {
        apiCall(
          `/api/burndown/business_value_burndown?project_id=${projectId}&sprint_id=${sprintId}`,
      ...
      if (authToken && projectId) {
        apiCall(
          `/api/burndown/business_value_burndown_for_all_sprints?project_id=${projectId}`,
      ...
      if (authToken && sprintId) {
        apiCall(
          `/api/burndown/partial_userstory_burndown?sprint_id=${sprintId}`,
      ...
      if (authToken && projectId) {
        apiCall(
          `/api/burndown/partial_story_points?project_id=${projectId}`,
      ...
      if (authToken && sprintId) {
        apiCall(
          `/api/burndown/userstory_burndown?sprint_id=${sprintId}`,
      ...
      if (authToken && projectId) {
        apiCall(
          `/api/burndown/userstory_burndown_for_all_sprints?project_id=${projectId}`,
  ...
  useEffect(() => {
    ...
    if (
      businessValueAllSprintData &&
      partialStoryPointAllSprintData &&
      fullStoryPointAllSprintData
    ) {
      ...
      updatedmultiChartAllSprintData.datasets.push(
        fullStoryPointAllSprintData.datasets[0]
      );
      updatedmultiChartAllSprintData.datasets.push(
        partialStoryPointAllSprintData.datasets[0]
      );
      updatedmultiChartAllSprintData.datasets.push(
        businessValueAllSprintData.datasets[0]
      );
